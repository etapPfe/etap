...
add : async (req, res) => {
  try {
    ...
    const newPointage = await db.Conge.create({ idUser, Name, Typecong, DD,DF,Raison });
...
// Controller to get a specific pointage record by Id
 getone : async (req, res) => {
  try {
    const { id } = req.body;
    const pointage = await db.Conge.findOne({ where: { Id: id } });
    if (pointage) {
      res.json(pointage);
    } else {
...
// Controller to get all pointage records
 getall :async (req, res) => {
  try {
    const pointages = await db.Conge.findAll();
    res.json(pointages);
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
